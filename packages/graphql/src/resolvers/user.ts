...
function addUser(phoneNumber, registrationToken, authToken="") {
  ...
  datastore
    .save(entity)
    .then(() => {
      console.log(`User ${phoneNumber} w reg token ${registrationToken} and auth token ${authToken} created successfully.`);
    })
    .catch(err => {
      console.error('ERROR:', err);
    });
...
export const userMutations = {
    ...
    authenticateUser: (_, { phoneNumber, registrationToken }) => {
        const user = queryHelper(datastore.createQuery('User').filter('phoneNumber', '=', phoneNumber).filter('registrationToken', '=', 'registrationToken'));
